...
var resize = {
	...
	poll: function() {
		// closeActive();
		resize.setDocWidth();
		if (resize.docWidth > 0 && resize.docWidth < 480) {
...
var navMain = {
	...
	initialise: function(container) {
		if ($(container).length) {
			...
			navMain.drop = $(navMain.container).find('.drop');
			$(navMain.container).find('a').on('mouseup', navMain.openClose);
		}
	},

	openClose: function() {
		$(navMain.drop).toggleClass('active');
